...
```{fit the model}
fit <- glm(Outbreak ~ Population_log + Avg_temp + Mean_drought + Basic_handwashing + Poverty_headcount,
...
plot5 <- plot_model(fit, type = "pred", title = "", terms = "Basic_handwashing") 
+ xlab("Basic Handwashing (%)")
```
...
best_glm <- glm(Outbreak ~ Mean_drought + Population_log + Slum_percent + Poverty_headcount + Avg_temp + Basic_sanitation + Withdrawal_percap, data = data_historical, family = binomial)
